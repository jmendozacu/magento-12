<?php
    ...
    class MW_Onestepcheckout_IndexController extends Mage_Checkout_OnepageController
    {
        ...
        public function saveAction()
        {
            ...
            if(isset($post['billing']['firstname'])){
                $post['billing']['firstname'] = addslashes($post['billing']['firstname']);
            }
            if(isset($post['billing']['lastname'])){
                $post['billing']['lastname'] = addslashes($post['billing']['lastname']);
            }
            if(isset($post['billing']['email'])){
                $post['billing']['email'] = addslashes($post['billing']['email']);
            }
            if(isset($post['billing']['company'])){
                $post['billing']['company'] = addslashes($post['billing']['company']);
            }
            if(isset($post['billing']['street'][0])){
                $post['billing']['street'][0] = addslashes($post['billing']['street'][0]);
            }
            if(isset($post['billing']['street'][1])){
                $post['billing']['street'][1] = addslashes($post['billing']['street'][1]);
            }
            if(isset($post['billing']['city'])){
                $post['billing']['city'] = addslashes($post['billing']['city']);
            }
            if(isset($post['billing']['region'])){
                $post['billing']['region'] = addslashes($post['billing']['region']);
            }
            if(isset($post['billing']['postcode'])){
                $post['billing']['postcode'] = addslashes($post['billing']['postcode']);
            }
            if(isset($post['billing']['telephone'])){
                $post['billing']['telephone'] = addslashes($post['billing']['telephone']);
            }

            if(isset($post['shipping']['firstname'])){
                $post['shipping']['firstname'] = addslashes($post['billing']['firstname']);
            }
            if(isset($post['shipping']['lastname'])){
                $post['shipping']['lastname'] = addslashes($post['billing']['lastname']);
            }
            if(isset($post['shipping']['email'])){
                $post['shipping']['email'] = addslashes($post['billing']['email']);
            }
            if(isset($post['shipping']['company'])){
                $post['shipping']['company'] = addslashes($post['shipping']['company']);
            }
            if(isset($post['shipping']['street'][0])){
                $post['shipping']['street'][0] = addslashes($post['shipping']['street'][0]);
            }
            if(isset($post['shipping']['street'][1])){
                $post['shipping']['street'][1] = addslashes($post['shipping']['street'][1]);
            }
            if(isset($post['shipping']['city'])){
                $post['shipping']['city'] = addslashes($post['shipping']['city']);
            }
            if(isset($post['shipping']['region'])){
                $post['shipping']['region'] = addslashes($post['shipping']['region']);
            }
            if(isset($post['shipping']['postcode'])){
                $post['shipping']['postcode'] = addslashes($post['shipping']['postcode']);
            }
            if(isset($post['shipping']['telephone'])){
                $post['shipping']['telephone'] = addslashes($post['shipping']['telephone']);
            }
            ...
            if($post) {
                ...
                if(isset($updates->updatepaymenttype)) {
                    ...
                    $block_payment_method = $layout->createBlock('checkout/onepage_payment_methods');
                    if(Mage::getEdition() == 'Enterprise'){
                        $block_html   = $block_payment_method->setAjax(1)->setTemplate('mw_onestepcheckout/daskboard/onepage/payment/methods.phtml')->toHtml();
                    }else{
                        $block_html   = $block_payment_method->setAjax(1)->toHtml();
                    }

                    $html['payment_method'] = $block_html;
                }
                if(isset($updates->updateusestorecredit)) {
                    $post_payment = Mage::app()->getRequest()->getPost('payment');
                    $this->_importPaymentData($post_payment['use_customer_balance']);
                    $this->getOnepage()->getQuote()->collectTotals()->save();
                }
                ...
                $html              = array(
                    ...
                    'items'               => $html_item,
                    'review_info'         => $block_onepage->renderView(),
        ...
        /**
         * This function use for using store credit
         *
         * @param Mage_Sales_Model_Quote $quote
         * @param Varien_Object|Mage_Sales_Model_Quote_Payment $payment
         * @param bool $shouldUseBalance
         */
        protected function _importPaymentData($shouldUseBalance)
        {
            $quote = $this->getQuote();
            ...
            if($paymentMethod == 'sagepayserver') {
                ...
                if($paymentMethod == "hosted_pro" || $paymentMethod == "payflow_link" || $paymentMethod == "payflow_advanced") {
                    ...
                    try {
                        ...
                        $orderId = $read->fetchOne('SELECT entity_id FROM sales_flat_order_grid ORDER BY entity_id DESC LIMIT 0 ,1');
                        ...
                        if(isset($orderId) && $orderId > 0){
                            ...
                            $__fields = array();
                            $__fields['order_from'] = 'Website';
                            $__where = $connection->quoteInto('entity_id=?', $orderId);
                            $connection->update('sales_flat_order_grid', $__fields, $__where);
