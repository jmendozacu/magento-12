<?php
    ...
    class MW_Onestepcheckout_Model_Observer_Layout extends Mage_Core_Model_Abstract
    {
        ...
        public function controllerLayoutBefore(Varien_Event_Observer $observer)
        {
            ...
            // $this->_path_js         = $path_phys . "/js/mw_onestepcheckout/onestep-config.js";
        ...
        public function createCustomCss()
        {
            ...
            foreach ($cssColor as $c)
            {
                if(strpos($c, "_") > -1)
                {
                    ...
                    $arrConvertColor[$c] = Mage::helper('onestepcheckout')->colourBrightness($color, $num[1] . "." . $num[2]);
                }
                else
                {
...
//                var_dump($cssColorGift); die();
            ...
            if(!isset($height_bar_value)){
                $height_bar_value = '';
            }
            if(!isset($background_pos_value)){
                $background_pos_value = '';
            }
            if(!isset($font_site_value)){
                $font_site_value = '';
            }

            if(!isset($checkout_height_value)){
                $checkout_height_value = '';
            }
            if(!isset($checkout_lineheight_value)){
                $checkout_lineheight_value = '';
            }
            if(!isset($checkout_font_value)){
                $checkout_font_value = '';
            }
        ...
        protected function createConfigFile()
        {
            ...
            $paramHttps  = (Mage::app()->getStore()->isCurrentlySecure()) ? array() : array();
            Mage::log($paramHttps,null,'debug12.log');
            $base_URLs   = preg_replace('/http:\/\//is', 'http://', $base_url[0]);
...
window.onestepConfig = {
    url: {
        ...
        saveOrder                   : '" . Mage::getUrl('checkout/onepage/saveOrder/form_key/' . Mage::getSingleton('core/session')->getFormKey(), $paramHttps) . "',
    ...
    giftwrap                : '" . Mage::getStoreConfig('onestepcheckout/addfield/price_gift_wrap', $store_id) . "',
