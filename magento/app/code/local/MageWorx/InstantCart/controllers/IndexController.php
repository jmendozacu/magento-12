    ...
    protected function _updateShoppingCart()
    {
        try {
            ...
            if (is_array($cartData)) {
                ...
                foreach ($cart->getQuote()->getAllItems() as $k=>$item) {
                    if(isset($cartData[$k]['qty'])){
                        $item->setQty($cartData[$k]['qty']);
                    }
            ...
            if (strlen($couponCode)) {
                if ($couponCode == $this->_getQuote()->getCouponCode()) {
                    $this->_getSession()->addSuccess(
                        Mage::helper('icart')->__js('Coupon code "%s" was applied.', Mage::helper('core')->htmlEscape($couponCode))
                    );
                }
                else {
                    $this->_getSession()->addError(
                        Mage::helper('icart')->__js('Coupon code "%s" is not valid.', Mage::helper('core')->htmlEscape($couponCode))
                    );
                }
            } else {
