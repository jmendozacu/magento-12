...
class AW_Advancedreports_Helper_Data extends AW_Advancedreports_Helper_Abstract
{
    ...
    const ROUTE_ADVANCED_SALESDETAILED = 'advanced_salesdetailed';
    const ROUTE_ADVANCED_USERGROUPS = 'advanced_usergroups';
    const ROUTE_ADVANCED_PURCHASED = 'advanced_purchased';
    const ROUTE_ADVANCED_STOCKVSSOLD = 'advanced_stockvssold';
    const ROUTE_ADVANCED_ABANDONED = 'advanced_abandoned';
    const ROUTE_ADVANCED_SALESSTATISTICS = 'advanced';
    ...
    public function getCustomizedValues()
    {
        ...
        foreach ($this->getQueue()->getStoredRanges() as $range) {
            $result[] = array(
                ...
                'from' => strftime(self::FRONTEND_STRFTIME_FORMAT, strtotime($dateHelper->fromTimestamp(@$range['report_from']))),
                'to' => strftime(self::FRONTEND_STRFTIME_FORMAT, strtotime($dateHelper->fromTimestamp(@$range['report_to']))),
    ...
    public function getChartParams($key)
    {
        ...
        $params[self::ROUTE_ADVANCED_SALESDETAILED] = array(
            array('value' => 'ordered_qty', 'label' => 'Quantity'),
        );
        ...
        $params[self::ROUTE_ADVANCED_STOCKVSSOLD] = array(
            array('value' => 'ordered_qty', 'label' => 'Quantity'),
        );

        $params[self::ROUTE_ADVANCED_ABANDONED] = array(
            array('value' => 'abandonment_rate', 'label' => 'Abandonment Rate'),
        );

        $params[self::ROUTE_ADVANCED_SALESSTATISTICS] = array(
            array('value' => 'avg_order_amount', 'label' => 'AVG Orders Amount'),
        );
    ...
    public function getNeedReload($key)
    {
        ...
        $params[self::ROUTE_ADVANCED_SALESDETAILED] = false;
        $params[self::ROUTE_ADVANCED_USERGROUPS] = true;
        $params[self::ROUTE_ADVANCED_PURCHASED] = false;
        $params[self::ROUTE_ADVANCED_STOCKVSSOLD] = false;
        $params[self::ROUTE_ADVANCED_ABANDONED] = false;
    ...
    public function getNeedTotal($key)
    {
        ...
        $params[self::ROUTE_ADVANCED_SALESDETAILED] = true;
        $params[self::ROUTE_ADVANCED_USERGROUPS] = true;
        $params[self::ROUTE_ADVANCED_PURCHASED] = true;
        $params[self::ROUTE_ADVANCED_STOCKVSSOLD] = true;
        $params[self::ROUTE_ADVANCED_ABANDONED] = true;
        return $params[$key];
    }
