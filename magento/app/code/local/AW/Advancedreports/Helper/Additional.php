...
class AW_Advancedreports_Helper_Additional extends AW_Advancedreports_Helper_Abstract
{
     ...
     * @param $name
     *
     * @return AW_Advancedreports_Model_Reports_Item
     */
    protected function _getItemByName($name)
    {
        $reports = $this->getReports()->getAdditionalReports();
        ...
        return new AW_Advancedreports_Model_Reports_Item();
    }
    ...
    public function getVersionCheck($item)
    {
        if (is_string($item)) {
            return version_compare(
                Mage::helper('advancedreports')->getVersion(), $this->_getItemByName($item)->getRequiredVersion(), '>='
            );
        } elseif ($item instanceof AW_Advancedreports_Model_Reports_Item) {
