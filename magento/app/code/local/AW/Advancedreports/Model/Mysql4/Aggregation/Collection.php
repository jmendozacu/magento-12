...
class AW_Advancedreports_Model_Mysql4_Aggregation_Collection extends Mage_Core_Model_Mysql4_Collection_Abstract
{
    ...
    protected function _getBetweenIntervals($periods, $from, $to)
    {
        $periods = array_merge(
            array(
                 array(
                     'from' => Mage::helper('advancedreports')->getDate()->incSec($to),
                 )
            ),
            $periods,
            array(
                 array(
                     'to' => Mage::helper('advancedreports')->getDate()->decSec($from),
                 )
            )
        );
        ...
        for ($i = $last; $i < $first; $i++) {
            $_from = Mage::helper('advancedreports')->getDate()->incSec($periods[$i + 1]['to']);
            $_to = Mage::helper('advancedreports')->getDate()->decSec($periods[$i]['from']);
            if ($_to > $_from) {
                $intervals[] = array(
                    'from' => $_from,
                    'to'   => $_to,
        ...
        if (($from = $period->getFrom()) && ($to = $period->getTo())) {
            ...
            foreach ($collection as $period) {
                ...
                if (($period->getFrom() < $from)) {
                    ...
                    $aggregating->setTo(Mage::helper('advancedreports')->getDate()->decSec($from));
                    $aggregating->save();
                }
                ...
                if (($period->getTo() > $to)) {
                    $aggregating = Mage::getModel('advancedreports/aggregation');
                    $aggregating->addData($aData);
                    $aggregating->setFrom(Mage::helper('advancedreports')->getDate()->incSec($to));
                    $aggregating->save();
                }
            }
        }
    ...
    {
        $periodCreatedAt = Mage::helper('advancedreports')->getDate()->getThisDayPeriod($createdAt);
        $periodOrigUpdatedAt = Mage::helper('advancedreports')->getDate()->getThisDayPeriod($origUpdatedAt);
        $periodUpdatedAt = Mage::helper('advancedreports')->getDate()->getThisDayPeriod($updatedAt);
