...
class Mage_Adminhtml_Sales_Order_CreateController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function saveAction()
    {
        try {
            ...
            /* Start Distiguish between order placed on Magento admin or frontend */
            ...
            if(isset($lastOrderId) && $lastOrderId > 0){
                ...
                $__fields = array();
                $__fields['order_from'] = 'Admin Panel';
                $__where = $connection->quoteInto('entity_id=?', $lastOrderId);
                $connection->update('sales_flat_order_grid', $__fields, $__where);
                 
                $connection->commit();
            }
            /* End Distiguish between order placed on Magento admin or frontend */
