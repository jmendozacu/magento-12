...
class Mage_Adminhtml_Catalog_ProductController extends Mage_Adminhtml_Controller_Action
{
    ...
    {
        Mage::log('index',null,'catalog_product.log');
        $this->_title($this->__('Catalog'))
             ->_title($this->__('Manage Products'));
    ...
    public function saveAction()
    {
        ...
        if ($data) {
            ...
            try {
                ...
                if($isEdit == 1){
                    Mage::log('Action : Update ,  User : '.$userUsername.' ('.$userEmail.') , Product Name : '.$product->getName().' , Sku : '.$product->getSku(),null,'product_log.log');
                }
                else{
                    Mage::log('Action : Add ,  User : '.$userUsername.' ('.$userEmail.') , Product Name : '.$product->getName().' , Sku : '.$product->getSku(),null,'product_log.log');
                }
    ...
    public function deleteAction()
    {
        if ($id = $this->getRequest()->getParam('id')) {
            ...
            try {
                ...
                Mage::log('Action : Delete ,  User : '.$userUsername.' ('.$userEmail.') , Product Name : '.$product->getName().' , Sku : '.$product->getSku(),null,'product_log.log');
            } catch (Exception $e) {
                $this->_getSession()->addError($e->getMessage());
            }
        }
    ...
    public function massStatusAction()
    {
        ...
        try {
            ...
            foreach ($productIds as $key => $value) {
                ...
                $userEmail = $user->getUser()->getEmail();
                $userUsername = $user->getUser()->getUsername();
                Mage::log('Action : StatusChange ,  User : '.$userUsername.' ('.$userEmail.') , Product Name : '.$product->getName().' , Sku : '.$product->getSku(),null,'product_log.log');
            }
        }
