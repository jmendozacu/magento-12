...
class TBT_Rewards_Model_Customer_Indexer_Points extends Mage_Index_Model_Indexer_Abstract
{
    ...
    protected $_matchedEntities = array (
        self::REWARDS_TRANSFER_ENTITY => array (Mage_Index_Model_Event::TYPE_SAVE ),
        self::REWARDS_CUSTOMER_ENTITY => array (Mage_Index_Model_Event::TYPE_SAVE )
    );
            ...
            try {
                if($event->getEntity() == self::REWARDS_TRANSFER_ENTITY) {
                    if ($event_model) {
                        ...
                        $customer = Mage::getModel('rewards/customer')->load($transfer->getCustomerId());
                        $this->_getResource ()->reindexUpdate ( $customer );
                        Mage::dispatchEvent('rewards_points_indexer_update', array('customer' => $customer, 'transfer' => $transfer));
                    ...
                    if ($event_model) {
                        ...
                        $this->_getResource ()->reindexUpdate ( $customer );
                        Mage::dispatchEvent('rewards_points_indexer_update', array('customer' => $customer));
                    }
                } else {
                ...
                $this->reindexAll ();
                Mage::dispatchEvent('rewards_points_indexer_reindex_all', array('indexer_event_model' => $event_model));
            }
        }
