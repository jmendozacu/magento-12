...
class TBT_Rewards_Model_Session extends Mage_Core_Model_Session_Abstract {
    ...
    /**
     * True, if a recurring order is being placed.
     * Check TBT_Rewards_Model_Sales_Recurring_Profile_Observer for more details.
     *
     * @return boolean
     */
    public function isRecurringOrderBeingPlaced()
    {
        if ($this->hasData('recurring_order_being_placed')) {
            return $this->getData('recurring_order_being_placed');
        }
    ...
    public function getCustomerId()
    {
        ...
        if (empty ( $customer_id )) {
            if ($this->isAdminMode ()) {
                ...
                // if registering an account on checkout, we need this, because the Customer session still
                // doesn't have the full customer with ID
                $customer_id = $this->_customer->getId();
            }
        }
    ...
    public function calculateCartPoints($rule_id, $order_items, $allow_redemptions, $prediction_mode = false) {
        ...
        if ($rule->getId ()) {
            ...
            if ($crActions->isGivePointsAction ( $rule->getPointsAction () )) {
                ...
                // if points to be earned are rewards AFTER discounts, subtract discount from total price
                if (Mage::helper ( 'rewards/config' )->calcCartPointsAfterDiscount ()){
                    // get total discount associated with the items that this rule applies to
                ...
                //@triley -a 12/03/12: Jira fix 1210
                /*if ($points_to_transfer < 0) {  //if the earned points balance is negative, then set to zero
                    $points_to_transfer = 0;
                }*/
                //--- End of Jira Fix 1210
                ...
                /* Give Point By Grand Tatal*/

                if(!$this->isAdminMode ()){
                    $grandTotal = floor(Mage::getModel('checkout/cart')->getQuote()->getGrandTotal());
                    Mage::log('grandTotal : '.$grandTotal,null,'grandTotal.log');
                }
                else{
                    $grandTotal = floor(Mage::getModel('adminhtml/sales_order_create')->getQuote()->getGrandTotal());    
                    Mage::log('grandTotal : '.$grandTotal,null,'grandTotal.log');
                }

                $pointAmountStep = $rule->getPointsAmountStep ();
                Mage::log('pointAmountStep : '.$pointAmountStep,null,'grandTotal.log');
                $points_to_transfer = $pointAmountStep * $grandTotal;
                Mage::log('points_to_transfer : '.$points_to_transfer,null,'grandTotal.log');
                ...
                if ($rule->getPointsDiscountAction () == 'by_percent') {
                    ...
                    $max_points = $rule->getPointsAmount () * ceil ( (($num_percents) / $rule->getPointsDiscountAmount ()) );
                } else {
    ...
    public function collectShoppingCartRedemptions($cart = null) {
        ...
        $applied_redemptions_original = Mage::getModel ( 'rewards/salesrule_list_valid_applied' )->initQuote ( $cart )->getList ();

        foreach ( $applied_redemptions_original as $rule_id ) {
            $rule = Mage::helper ( 'rewards/transfer' )->getSalesRule ( $rule_id );
    ...
    public function getTotalPointsEarnedOnCart($cart = null) {
        ...
        $grandArray[0]->points_amt = floor($grandTotal);
        
        if ($this->calculateAccumulatedPoints ( $grandArray, $points )) {
            $points_exist = true;
        }*/
     ...
     * Essentially copied from TBT_Rewards_Block_Checkout_Onepage_Link
     *
     * @return boolean
     */
    public function canCheckoutWithCurrentRedemptions()
    {
        if ($this->isCustomerLoggedIn()) {
            if ($this->isCartOverspent()) {
                return false;
            }
        } else {
